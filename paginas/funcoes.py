        ...
        with st.status('Processando PDF para identificar páginas para OCR...', expanded=True) as status:
            ...
            for pagina in leitor.pages:
                ...
                if len(texto.strip()) > limiar_texto:
                    ...
                    status.write(f'(PyPDF2) Texto da Página {numero_da_pagina+1} extraída com sucesso.')
                else:
                # Usar fitz aqui para verificar se contém imagem. Se conter imagem, marca o número da página. Se não, deixa o conteúdo de texto extraído mesmo.
                    ...
                    if pagina_fitz.get_images(full=True):
                        status.write(f'(Fitz) Página {numero_da_pagina+1} tem pouco texto E contém imagem. Marcando para OCR.')
                        ...
                        status.write(f'(Fitz) Página {numero_da_pagina+1} tem pouco texto mas NÃO contém imagem. Mantendo texto original.')
                        pagina_apenas_texto.append(f'Página {numero_da_pagina+1}: {texto}')
                numero_da_pagina += 1
            
            doc_fitz.close()
            status.write(f"PyPDF2 encontrou {numero_da_pagina} páginas no total.")
            status.write(f"Páginas marcadas para OCR: {numero_da_pagina_com_imagem}")
