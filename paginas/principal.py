...
if nome_do_modelo_selecionado != 'Nenhum (Criar novo)':
    ...
    with st.expander(label = 'Clique aqui para visualizá-las',expanded=False, icon = '🔎'):
        st.table(st.session_state.get('lista_de_variaveis', []).style.hide(axis='index'), border='horizontal')
    ...
    if not st.session_state.lista_de_variaveis:
        ...
        with st.expander(label = 'Clique aqui para visualizá-las',expanded=False, icon = '🔎'):
            st.table(st.session_state.lista_de_variaveis.style.hide(axis='index'), border='horizontal')
